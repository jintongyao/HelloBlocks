 ...
 *  @return BlockSprite
 */
BlockSprite* BlockSprite::createBlock(bool isTheOneFlag, int red, int green, int blue, int width, int height, int posX, int posY) {
    BlockSprite *block = new BlockSprite();
    if (block && block->init()) {
        block->autorelease();
        block->blockInit(isTheOneFlag, red, green, blue, width, height, posX, posY);
        return block;
    }
    CC_SAFE_DELETE(block);
    return NULL;
 ...
 *  @param posY
 */
void BlockSprite::blockInit(bool isTheOneFlag, int red, int green, int blue, int width, int height, int posX, int posY) {
    //this->setBlockColor(new MyColor(colorTheme, ranColor));
    ...
    if(isTheOneFlag) {
        ...
        switch (randomInt) {
            case 1:
                this->blockBackgroundLayer = LayerColor::create(Color4B(red + 15, green, blue, 255));
                break;
            case 2:
                this->blockBackgroundLayer = LayerColor::create(Color4B(red, green + 15, blue, 255));
                break;
            case 3:
                this->blockBackgroundLayer = LayerColor::create(Color4B(red, green, blue + 15, 255));
                break;
            case 4:
                this->blockBackgroundLayer = LayerColor::create(Color4B(red - 15, green, blue, 255));
                break;
        ...
        this->blockBackgroundLayer = LayerColor::create(Color4B(red, green, blue, 255));
    }
    this->blockBackgroundLayer->setPosition(Point(posX, posY));
    this->blockBackgroundLayer->setContentSize(Size(width, height));
    this->addChild(blockBackgroundLayer);
